...
class RideDetailsScreen extends StatefulWidget {
  RideDetailsScreen({Key? key, required this.ride, required this.user});
  Ride ride;
  UserC user;
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: SingleChildScrollView(
          child: Column(
            children: [
              TopNavigationCustom(leftIcon: Icons.arrow_back, mainText: "Details", rightIcon: null),
              StreamBuilder(
                stream: _streamRide,
...
class DetailsCard1 extends StatelessWidget {
  const DetailsCard1({Key? key, required this.screenWidth, required this.ride, required this.user});
...
class DetailsCard2 extends StatelessWidget {
  const DetailsCard2({Key? key, required this.screenWidth, required this.ride, required this.user});
...
class DetailsCard3 extends StatelessWidget {
  const DetailsCard3({Key? key, required this.screenWidth, required this.ride});
  final Ride ride;
  final double screenWidth;
  ...
  Widget build(BuildContext context) {
    ...
    itemsList = [
      ...
      {"icon": Icons.add_road, "text": ride.highway ? "Traveling on the highway" : "Not traveling on the highway"},
      {
        "icon": Icons.two_wheeler,
        "text": ride.acceptType == "-" ? "Accepting all types of motorcycles" : "Preferring type: ${ride.acceptType.toLowerCase()}"
      },
      {
        "icon": Icons.people_alt,
        "text": ride.maxPeople == 0 ? "No limit on the number of people" : "Max number of people in the group: ${ride.maxPeople}"
      },
...
class DetailsCard4 extends StatefulWidget {
  const DetailsCard4({Key? key, required this.screenWidth, required this.streamRiders, required this.user, required this.ride});
                ...
                children: [
                  SizedBox(
                    ...
                    child: riders.length == 1
                        ? const Center(child: Text("No other bikers on this ride yet."))
                        : ListView.builder(
                        ...
                        return Container(
                          ...
                          child: isRiderInRide
                              ? LargeButtonCustom(
                                  onTap: () async {
                                    if (widget.ride.userId == FirebaseAuth.instance.currentUser!.uid) {
                                      ...
                                      Fluttertoast.showToast(
                                        msg: res == true ? "Successfully removed!" : "Error. Please try again.",
                                  ...
                                  btnText:
                                      widget.ride.userId == FirebaseAuth.instance.currentUser!.uid ? "Edit ride" : "Cancel ride",
                                    ...
                                    widget.ride.maxPeople == 0 || riders.length != (widget.ride.maxPeople + 1) // 0 = no people limit
                                        ? LargeButtonCustom(
                                            onTap: () async {
                                              ...
                                              Fluttertoast.showToast(
                                                msg: res == true ? "Successfully joined!" : "Error. Please try again.",
                                                toastLength: Toast.LENGTH_SHORT,
                                                gravity: ToastGravity.BOTTOM,
                                                backgroundColor: res == true ? const Color(0xFF528C9E) : const Color(0xFFA41723),
                                                textColor: Colors.white,
